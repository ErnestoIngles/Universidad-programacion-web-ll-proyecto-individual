...
const Paginacion = ({
  ...
  const maximoPaginacionAMostrar = 3;

  let paginaInicio = Math.max(
    1,
    paginaActual - Math.floor(maximoPaginacionAMostrar / 2)
  );

  let paginaFin = Math.min(
    totalPaginas,
    paginaInicio + maximoPaginacionAMostrar - 1
  );

  if (paginaFin - paginaInicio + 1 < maximoPaginacionAMostrar) {
    paginaInicio = Math.max(
      1,
      paginaFin - maximoPaginacionAMostrar + 1
    );
  }
